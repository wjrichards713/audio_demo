...
public class WavWriter {
  ...
  public boolean start(String name, Context context) {
    outPath = new File(context.getCacheDir(), name + ".wav");
    try {
  ...
  public void pushAudioBytes(byte[] audioBytes, int numOfBytes) {
    if (out == null) {
      Log.w(TAG, "pushAudioBytes(): Error writing " + outPath + "  null pointer");
      return;
    }

    try {
      out.write(audioBytes, 0, numOfBytes);  // Write the byte array directly
      framesWritten += numOfBytes / (RECORDER_BPP / 8);  // Update frame count
    } catch (IOException e) {
      Log.w(TAG, "pushAudioBytes(): Error writing " + outPath, e);
      out = null;
    }
  }
